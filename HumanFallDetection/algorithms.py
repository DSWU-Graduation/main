...
def extract_keypoints_parallel(queue, args, self_counter, other_counter, consecutive_frames, event):
    ...
    while not event.is_set():
        ...
        if tagged_df is None:
            ...
            # CSV는 초 단위만 있을 수 있으므로 frame 기반 보정 추가
            curr_time += frame * (1.0 / args.fps)
        ...
        dict_vis = {"img": img, "keypoint_sets": keypoint_sets, "width": width, "height": height, "vis_keypoints": True,
                    "vis_skeleton": True, "CocoPointsOn": args.coco_points,
...
def alg2_sequential(queues, argss, consecutive_frames, event):
    ...
    model.load_state_dict(torch.load('HumanFallDetection/model/lstm_weights.sav', map_location=argss[0].device))
    model.eval()
    ...
    while True:
        ...
        if not any(q.empty() for q in queues):
            ...
            if argss[0].num_cams == 1:
                ...
                dict_frames[0]["overlay"] = img
                queues[0].put(dict_frames[0])
                # print(img1.shape)
                ...
                dict_frames[0]["overlay"] = img1
                dict_frames[1]["overlay"] = img2
                queues[0].put(dict_frames[0])
                queues[1].put(dict_frames[1])
                # print(img1.shape)
